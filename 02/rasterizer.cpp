...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    float xmi = std::min(std::min(v[0].x(), v[1].x()), v[2].x());
    float ymi = std::min(std::min(v[0].y(), v[1].y()), v[2].y());
    float xma = std::max(std::max(v[0].x(), v[1].x()), v[2].x());
    float yma = std::max(std::max(v[0].y(), v[1].y()), v[2].y());
    ...
    for (int x = xmi; x < xma; x++) {
        for (int y = ymi; y < yma; y++) {
            int cnt = 0;
            if (MSAAx4) {
                cnt = int(insideTriangle(x + 0.25, y + 0.25, t.v)) +
                    int(insideTriangle(x + 0.25, y - 0.25, t.v)) +
                    int(insideTriangle(x - 0.25, y + 0.25, t.v)) +
                    int(insideTriangle(x - 0.25, y - 0.25, t.v));
            }
            else {
                cnt = int(insideTriangle(x, y, t.v)) * 4;
            }
            if (cnt == 0) continue;
